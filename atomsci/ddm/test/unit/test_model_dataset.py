...
if not datastore_is_down:
    (params_from_ds, dataset_obj_from_datastore, df_datastore) = (
        utils.datastore_objects()
    )
...
def test_create_model_dataset():
    ...
    (params_from_file, dataset_obj_from_file, df_delaney) = utils.delaney_objects()
    (params_from_file_scaffold, dataset_obj_from_file_scaffold, df_delaney) = (
        utils.delaney_objects(split_strategy="train_valid_test", splitter="scaffold")
    )
    ...
    if not datastore_is_down:
        test_list.append(
            isinstance(dataset_obj_from_datastore, model_dataset.DatastoreDataset)
        )
    ...
    test_list.append(
        isinstance(dataset_obj_from_file.featurization, feat.DynamicFeaturization)
    )
    test_list.append(
        isinstance(dataset_obj_from_file.splitting, split.TrainValidTestSplitting)
    )
...
def test_get_dataset_tasks():
    ...
    subset_delaney = delaney_from_disk[
        [
            "Compound ID",
            "smiles",
    ...
    if not datastore_is_down:
        ...
        test_list.append(
            dataset_obj_from_datastore.tasks == params_from_ds.response_cols
        )
...
def test_get_featurized_data():
    ...
    test_list.append(
        isinstance(dataset_obj_from_file.dataset, dc.data.datasets.NumpyDataset)
    )
    test_list.append(len(dataset_obj_from_file.dataset) == len(df_delaney))
    test_list.append(
        dataset_obj_from_file.n_features == dataset_obj_from_file.params.ecfp_size
    )
    assert all(test_list)
...
def test_get_featurized_data_scaffold():
    ...
    test_list.append(
        isinstance(
            dataset_obj_from_file_scaffold.dataset, dc.data.datasets.NumpyDataset
        )
    )
    ...
    test_list.append(
        len(dataset_obj_from_file.dataset.y) == len(dataset_obj_from_file.dataset.ids)
    )
...
def test_split_dataset():
    ...
    test_list.append(
        set(dataset_obj_from_file.test_dset.ids.tolist())
        == set(dataset_obj_from_file.test_attr.index.tolist())
    )
...
def test_load_presplit_dataset():
    ...
    (params_from_file2, dataset_obj_from_file2, df_delaney2) = utils.delaney_objects(
        split_uuid=dataset_obj_from_file.split_uuid
    )
    ...
    test_list.append(
        (
            sorted(dataset_obj_from_file.test_dset.y)
            == sorted(dataset_obj_from_file2.test_dset.y)
        )
    )
    test_list.append(
        set(dataset_obj_from_file.test_attr.index.values)
        == set(dataset_obj_from_file2.test_attr.index.values)
    )
...
def test_combine_training_data():
    ...
    assert (
        concat_train_valid == dataset_obj_from_file.combined_train_valid_data.ids
    ).all()
...
def test_split_dataset_scaffold():
    ...
    test_list.append(
        len(dataset_obj_from_file.dataset)
        == len(train) + len(valid) + len(dataset_obj_from_file_scaffold.test_dset)
    )
    test_list.append(set(train.ids.tolist()) == set(train_attr.index.tolist()))
    test_list.append(set(valid.ids.tolist()) == set(valid_attr.index.tolist()))
    test_list.append(
        set(dataset_obj_from_file_scaffold.test_dset.ids.tolist())
        == set(dataset_obj_from_file_scaffold.test_attr.index.tolist())
    )
    assert all(test_list)
...
def test_combine_training_data_scaffold():
    ...
    test_list = []
    test_list.append(
        isinstance(dataset_obj_from_file_scaffold.combined_train_valid_data, DD)
    )
    test_list.append(
    ...
    test_list.append(
        (
            concat_train_valid
            == dataset_obj_from_file_scaffold.combined_train_valid_data.ids
        ).all()
    )
...
def test_get_split_metadata():
    ...
    test_list = []
    test_list.append(
        out_dict["split_strategy"] == dataset_obj_from_file.params.split_strategy
    )
    ...
    test_list.append(
        out_dict["split_valid_frac"] == dataset_obj_from_file.params.split_valid_frac
    )
    test_list.append(
        out_dict["split_test_frac"] == dataset_obj_from_file.params.split_test_frac
    )
...
def test_load_presplit_dataset():
    # open the test
    with open(
        tu.relative_to_file(__file__, "../test_datasets/H1_hybrid.json"), "r"
    ) as f:
        config = json.load(f)

    # change to some fake uuid
    config["split_uuid"] = "c63c6d89-8832-4434-b27a-17213bd6ef8"
    config["dataset_key"] = tu.relative_to_file(
