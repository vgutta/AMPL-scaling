...
@pytest.mark.basic
def test_kfold():
    json_file = tu.relative_to_file(__file__, "nn_ecfp_kfold.json")

    pparams = parse.wrapper(["--config_file", json_file])
    pparams.dataset_key = tu.relative_to_file(
...
@pytest.mark.basic
def test_graphconv():
    json_file = tu.relative_to_file(__file__, "nn_graph.json")
...
@pytest.mark.basic
def test_ecfp_nn():
    json_file = tu.relative_to_file(__file__, "nn_ecfp.json")
...
def test_train_valid_test():
    json_file = tu.relative_to_file(__file__, "nn_ecfp_random.json")
...
def test_attentivefp():
    json_file = tu.relative_to_file(__file__, "attentivefp_random.json")
...
@pytest.mark.basic
def test_gcnmodel():
    json_file = tu.relative_to_file(__file__, "gcnmodel_random.json")
...
def test_graphconvmodel():
    json_file = tu.relative_to_file(__file__, "GraphConvModel_random.json")
...
@pytest.mark.basic
def test_mpnnmodel():
    json_file = tu.relative_to_file(__file__, "MPNNModel_random.json")
...
def test_pytorchmpnnmodel():
    json_file = tu.relative_to_file(__file__, "PytorchMPNNModel_random.json")
...
def saved_model_identity(pparams):
    ...
    split_csv = tu.relative_to_file(
        __file__, "../../test_datasets/", train_pipe.data._get_split_key()
    )
    ...
    pred_df2 = pfm.predict_from_model_file(
        model_tar,
        test_df,
        id_col=id_col,
        smiles_col=smiles_col,
