...
def curate():
    ...
    if (
        ...
        assert (
            curated_df.shape[0] == 1116
        ), f"Error: Incorrect number of compounds ({len(curated_df)}, should be 1116)"
...
def test():
    ...
    reload_dir = (
        "result/delaney-processed_curated_fit/NN_graphconv_scaffold_regression/" + uuid
    )
    ...
    assert (
        os.path.isfile("delaney-processed_curated_predict.csv")
