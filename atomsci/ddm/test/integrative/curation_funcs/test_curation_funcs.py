...
test_files = [
    f"{script_path}/{test_file_prefix}-{suffix}.csv"
    for suffix in ["filtered", "aggregated", "averaged"]
]
...
def get_raw_data():
    ...
    dset_path = tu.relative_to_file(
        __file__, "../../test_datasets/pGP_MDCK_efflux_ratio_chembl29.csv"
    )
...
def test_remove_outlier_replicates():
    ...
    raw_df = get_raw_data()
    print(
        f"Raw data has {len(raw_df)} rows, {len(set(raw_df.base_rdkit_smiles.values))} unique compounds"
    )
    filt_df = curate_data.remove_outlier_replicates(
        raw_df,
        response_col="log_efflux_ratio",
        id_col="base_rdkit_smiles",
...
def test_aggregate_assay_data(filt_df=None):
    ...
    agg_df = curate_data.aggregate_assay_data(
        filt_df,
        value_col="log_efflux_ratio",
        label_actives=False,
        id_col="compound_id",
...
def test_average_and_remove_duplicates():
    ...
    print(
        f"Averaged data has {len(curated_df)} rows, {len(set(curated_df.base_rdkit_smiles.values))} unique compounds"
    )
