...
config_path = tu.relative_to_file(__file__, "config_list_inputs.json")
filter_dict_path = tu.relative_to_file(__file__, "filter_dict_example.json")
required_inputs = [
    "--dataset_key",
    "/ds/data/public/delaney/delaney-processed.csv",
    "--bucket",
    "gsk_ml",
]
wrong_type_inputs = [
...
list_inputs = [
    ...
    "--layer_sizes",
    "1000,500 300,300,300 20000,50,50",
    "--learning_rate",
    "1",
    "--weight_init_stddevs",
    "0.001,0.001 0.02,0.02,0.5 0.3,0.3,0.3",
...
required_inputs_namespace = argparse.Namespace(
    dataset_key="/ds/data/public/delaney/delaney-processed.csv", bucket="gsk_ml"
)
undefined_inputs_namespace = argparse.Namespace(
...
#                                                 bucket=5)
list_inputs_namespace = argparse.Namespace(
    dataset_key="/ds/data/public/delaney/delaney-processed.csv",
    bucket="gsk_ml",
    descriptor_type="moe",
...
hierarchical_input_dict = {
    "ModelMetadata": {
        ...
        "ModelSpecific": {"system": "twintron-blue", "previously_split": True},
    },
    "ModelTrainSelectionParameters": {
        ...
        "IgnoreThisHeading": {"model_type": "NN,RF"},
    },
}
...
def test_default_params_json():
    params = parse.wrapper(tu.relative_to_file(__file__, "config_required_inputs.json"))
    defaults = default_parameters()
    defaults.config_file = tu.relative_to_file(__file__, "config_required_inputs.json")
    test = []
...
def test_dupe_params_json(caplog):
    params = parse.wrapper(tu.relative_to_file(__file__, "config_dupe_inputs.json"))
...
def test_incorrect_params_json(caplog):
    ...
    params = parse.wrapper(tu.relative_to_file(__file__, "config_wrong_inputs.json"))
    for record in caplog.records:
        assert record.levelname == "WARNING"
...
def test_correct_input_type_json():
    params = parse.wrapper(tu.relative_to_file(__file__, "config_list_inputs.json"))
    test = []
    test.append(params.system == "twintron-blue")
    test.append(params.dataset_key == "/ds/data/public/delaney/delaney-processed.csv")
    test.append(params.layer_sizes == [[42, 42]])
...
def test_correct_input_type_command():
    ...
    test.append(
        params.bias_init_consts == [[1.0, 1.0], [2.0, 2.0, 2.0], [0.3, 0.3, 0.3]]
    )
