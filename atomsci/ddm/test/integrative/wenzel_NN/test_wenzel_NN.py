...
def clean():
    ...
    for f in [
        "hlm_clearance_curated_predict.csv",
        "hlm_clearance_curated_external.csv",
        "hlm_clearance_curated_fit.csv",
        "hlm_clearance_curated.csv",
    ]:
        if os.path.isfile(tu.relative_to_file(__file__, f)):
            os.remove(tu.relative_to_file(__file__, f))
...
def curate():
    ...
    with zipfile.ZipFile(
        tu.relative_to_file(__file__, "ci8b00785_si_001.zip"), "r"
    ) as zip_ref:
        zip_ref.extractall(tu.relative_to_file(__file__, "clearance"))

    raw_df = pd.read_csv(
        tu.relative_to_file(
            __file__, "clearance/SuppInfo/Dataset_chembl_clearcaco.txt"
        ),
        sep=";",
        dtype="str",
    )
    ...
    raw_df["hlm_clearance[mL.min-1.g-1]"] = raw_df[
        "hlm_clearance[mL.min-1.g-1]"
    ].str.replace(",", ".")
    ...
    hlmc_df = raw_df.rename(
        columns={"hlm_clearance[mL.min-1.g-1]": "value"}, inplace=False
    )
    hlmc_df.rename(columns={"ID": "compound_id"}, inplace=True)
    hlmc_df["rdkit_smiles"] = hlmc_df["Canonical_Smiles"].apply(
        struct_utils.base_smiles_from_smiles
    )
    ...
    curated_df.to_csv(tu.relative_to_file(__file__, data_filename), index=False)

    # Create second test set by reproducible index for prediction
    curated_df.tail(4989).to_csv(
        tu.relative_to_file(__file__, "hlm_clearance_curated_fit.csv")
    )
    curated_df.head(348).to_csv(
        tu.relative_to_file(__file__, "hlm_clearance_curated_external.csv")
    )
...
def download():
    ...
    if not os.path.isfile(tu.relative_to_file(__file__, "ci8b00785_si_001.zip")):
        integrative_utilities.download_save(
            "https://pubs.acs.org/doi/suppl/10.1021/acs.jcim.8b00785/suppl_file/ci8b00785_si_001.zip",
            tu.relative_to_file(__file__, "ci8b00785_si_001.zip"),
            verify=False,
        )

    assert os.path.isfile(tu.relative_to_file(__file__, "ci8b00785_si_001.zip"))
...
def test():
    ...
    with open(tu.relative_to_file(__file__, "config_wenzel_fit_NN.json")) as f:
        config = json.loads(f.read())

    config["result_dir"] = tu.relative_to_file(__file__, config["result_dir"])
    ...
    uuid = integrative_utilities.get_subdirectory(
        tu.relative_to_file(
            __file__,
            "result/hlm_clearance_curated_fit/NN_computed_descriptors_scaffold_regression",
        )
    )
    reload_dir = (
        tu.relative_to_file(
    ...
    with open(tu.relative_to_file(__file__, "config_wenzel_predict_NN.json"), "r") as f:
    ...
    predict_parameters.result_dir = tu.relative_to_file(
        __file__, predict_parameters.result_dir
    )
    ...
    data = data[
        [
            predict_parameters.id_col,
            predict_parameters.smiles_col,
    ...
    combined.to_csv(tu.relative_to_file(__file__, "hlm_clearance_curated_predict.csv"))
    assert (
        os.path.isfile(
            tu.relative_to_file(__file__, "hlm_clearance_curated_predict.csv")
        )
        and os.path.getsize(
            tu.relative_to_file(__file__, "hlm_clearance_curated_predict.csv")
        )
