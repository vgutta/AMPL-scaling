...
def init_data():
    ...
    if not os.path.exists(tu.relative_to_file(__file__, "data")):
        os.makedirs(tu.relative_to_file(__file__, "data"))

    shutil.copyfile(
        tu.relative_to_file(__file__, "../../test_datasets/MRP3_dataset.csv"),
        "data/MRP3_dataset.csv",
    )
    shutil.copyfile(
        tu.relative_to_file(
            __file__, "../../test_datasets/delaney-processed_curated_fit.csv"
        ),
        tu.relative_to_file(__file__, "data/delaney-processed_curated_fit.csv"),
    )
...
def clean():
    ...
    if "shortlist_test" in os.listdir():
        shutil.rmtree(tu.relative_to_file(__file__, "shortlist_test"))

    if "logs" in os.listdir():
        shutil.rmtree(tu.relative_to_file(__file__, "logs"))

    if "run.sh" in os.listdir():
        os.remove(tu.relative_to_file(__file__, "run.sh"))

    if "slurm_files" in os.listdir():
        shutil.rmtree(tu.relative_to_file(__file__, "slurm_files"))

    if "test_shortlist_with_uuids.csv" in os.listdir():
        os.remove(tu.relative_to_file(__file__, "test_shortlist_with_uuids.csv"))

    if "data" in os.listdir():
        shutil.rmtree(tu.relative_to_file(__file__, "data"))
...
def wait_to_finish(split_json, search_json, max_time=1200):
    ...
    shortlist_path = tu.relative_to_file(__file__, f"{slkey}.csv")
    print(shortlist_path)
    features = pparams.descriptor_type
    shortlist_df = pd.read_csv(shortlist_path)
    dataset_key = (
        shortlist_df["dataset_key"]
        .iloc[-1]
        .replace(".csv", f"_with_{features}_descriptors.csv")
    )
...
def test():
    ...
    if llnl_utils.is_lc_system():
        result_df = wait_to_finish(
            "test_shortlist_split_config.json",
            "test_shortlist_RF-NN-XG_hyperconfig.json",
            max_time=-1,
        )
