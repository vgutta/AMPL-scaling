...
def all_similar_tests(json_f, prefix="delaney-processed"):
    ...
    model_info = cm.get_best_perf_table(
        metric_type="r2_score", model_uuid=model_uuid, result_dir="result"
    )
...
def test_RF_results():
    ...
    assert json.loads(model_info["model_parameters_dict"]) == {
        "rf_estimators": 501,
        "rf_max_features": 33,
        "rf_max_depth": 10000,
    }
...
def test_AttentiveFP_results():
    ...
    assert json.loads(model_info["feat_parameters_dict"]) == {
