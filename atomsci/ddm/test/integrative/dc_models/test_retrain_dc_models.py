...
def H1_curate():
    ...
    if (
        ...
        curated_df["exact_mol_weight"] = curated_df["base_rdkit_smiles"].apply(
            lambda x: exact_mol_weight(x)
        )

        # make a really easy test. count the number of atoms in a molecule
        curated_df["num_atoms"] = curated_df["base_rdkit_smiles"].apply(
        ...
        curated_external_df = curated_df[
...
def train_and_predict(train_json_f, prefix="delaney-processed"):
    ...
    if prediction_type == "regression":
        ...
        integrative_utilities.training_statistics_file(
            reload_dir, "test", threshold, "r2_score"
        )
        ...
        if "perf_threshold" in config:
            threshold = float(config["perf_threshold"])
        integrative_utilities.training_statistics_file(
            reload_dir, "test", threshold, "accuracy_score"
        )
        score = integrative_utilities.read_training_statistics_file(
    ...
    predict = pfm.predict_from_model_file(
        tar_f,
        data,
