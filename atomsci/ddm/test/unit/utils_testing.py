...
def clean():
    split_files = glob.glob(tu.relative_to_file(__file__, "*_train_valid_test_*"))
    for f in split_files:
        os.remove(f)

    if os.path.exists(tu.relative_to_file(__file__, "pytest")):
        shutil.rmtree(tu.relative_to_file(__file__, "pytest"))
...
def delaney_objects(
    ...
    pdb.set_trace()
    delaney_inp_file = tu.relative_to_file(__file__, "config_delaney.json")
...
def datastore_objects(y=["PIC50"]):
    params_from_ds = parse.wrapper(
        tu.relative_to_file(__file__, "config_datastore_dset_cav12.json")
    )
...
def uncurated_objects(y=["VALUE_NUM"]):
    params_from_ds = parse.wrapper(
        tu.relative_to_file(__file__, "config_uncurated_bp.json")
    )
    params_from_ds.response_cols = y
    featurization = feat.create_featurization(params_from_ds)
    data = model_dataset.create_model_dataset(params_from_ds, featurization)
...
def moe_descriptors(datastore=False):
    if datastore == True:
        params_ds = parse.wrapper(
            tu.relative_to_file(__file__, "config_MAOA_moe_descriptors_ds.json")
        )
    else:
        params_file = parse.wrapper(
            tu.relative_to_file(__file__, "config_MAOA_moe_descriptors.json")
        )
    ...
    dataset_obj_for_desc = model_dataset.create_model_dataset(
        params_desc, featurization, ds_client=None
    )
