...
def test_predict_from_model():
    ...
    csv_path = tu.relative_to_file(
        __file__, "../../examples/BSEP/data/ChEMBL25_BSEP_curated_data.csv"
    )
    ...
    score = skm.accuracy_score(
        shuffled_df[response_col].values, pred_df[response_col + "_pred"].values
    )
...
def test_predict_full_dataset():
    """test that predict_full_dataset makes predictions in the same order as the input"""
    if not llnl_utils.is_lc_system():
        assert True
        return
    ...
    input_df, pred_params = pfm._prepare_input_data(
        shuffled_df, id_col, smiles_col, response_col, conc_col, dont_standardize
    )
